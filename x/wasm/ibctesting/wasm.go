...
func (chain *TestChain) StoreCode(byteCode []byte) types.MsgStoreCodeResponse {
	...
	fmt.Println("SENDER ACC")
	fmt.Println(chain.SenderAccount.GetAccountNumber())
	fmt.Println(chain.SenderAccount.GetSequence())
	fmt.Println(chain.SenderAccount.GetAddress().String())
...
func (chain *TestChain) InstantiateContract(codeID uint64, initMsg []byte) sdk.AccAddress {
	instantiateMsg := &types.MsgInstantiateContract{
		...
		CodeID:    codeID,
		InitMsg:   initMsg,
		InitCoins: sdk.NewCoins(sdk.NewCoin(coretypes.MicroLunaDenom, sdk.NewInt(100))),
	}
	...
	a, err := sdk.AccAddressFromBech32(pExecResp.ContractAddress)
	require.NoError(chain.t, err)
	return a
}
...
func (chain *TestChain) SmartQuery(contractAddr string, queryMsg interface{}, response interface{}) error {
	...
	req := types.QueryContractStoreRequest{
	...
	res := chain.App.Query(abci.RequestQuery{
		//Path: "/cosmwasm.wasm.v1.Query/SmartContractState", // FIXME: why abci?
		Path: "/terra/wasm/v1beta1/contracts/" + contractAddr + "/store",
		Data: reqBin,
	})
	...
	// unpack protobuf
	var resp types.QueryContractStoreResponse
	err = proto.Unmarshal(res.Value, &resp)
	if err != nil {
		return err
	}
	// unpack json content
	return json.Unmarshal(resp.QueryResult, response)
}
...
func (chain *TestChain) ContractInfo(contractAddr sdk.AccAddress) *types.ContractInfo {
	type testSupporter interface {
		TestSupport() *helpers.TestSupport
	}
	ci, err := chain.App.(testSupporter).TestSupport().WasmKeeper().GetContractInfo(chain.GetContext(), contractAddr)
	if err != nil {
		panic(err)
	}
	return &ci
