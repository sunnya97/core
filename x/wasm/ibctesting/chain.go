...
func NewTestChain(t *testing.T, coord *Coordinator, chainID string /*, opts ...wasm.Option*/) *TestChain { // FIXME: remove opts
	...
	acc := authtypes.NewBaseAccount(senderPrivKey.PubKey().Address().Bytes(), senderPrivKey.PubKey(), 6, 0)
	amount, ok := sdk.NewIntFromString("10000000000000000000")
	require.True(t, ok)

	balance := banktypes.Balance{
		Address: acc.GetAddress().String(),
		Coins:   sdk.NewCoins(sdk.NewCoin(coretypes.MicroLunaDenom, amount)),
	}

	app := NewTestingAppDecorator(t, helpers.SetupWithGenesisValSet(t, valSet, []authtypes.GenesisAccount{acc} /* opts,*/, balance))
...
func (chain *TestChain) SendMsgs(msgs ...sdk.Msg) (*sdk.Result, error) {
	...
	_, r, err := helpers.SignAndDeliver(
		chain.t,
		chain.TxConfig,
...
func (chain TestChain) GetTestSupport() *helpers.TestSupport {
...
func NewTestingAppDecorator(t *testing.T, terraApp *terraapp.TerraApp) *TestingAppDecorator {
	return &TestingAppDecorator{TerraApp: terraApp, t: t}
}
...
func (a TestingAppDecorator) TestSupport() *helpers.TestSupport {
	return helpers.NewTestSupport(a.t, a.TerraApp)
}
